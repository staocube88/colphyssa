...
if( !function_exists('adifier_count_compared') ){
function adifier_count_compared(){
	if( !empty( $_COOKIE['adifier_compare'] ) && $_COOKIE['adifier_compare'] != '-1' ){
		$list = json_decode( stripslashes( $_COOKIE['adifier_compare'] ), true );
		if( !empty( $list ) ){
			echo '<span class="unread-badge">'.sizeof( $list ).'</span>';
		}
	}
}
}
	...
	if( !empty( $list ) ){
		...
		if( $ads->have_posts() ){			
			...
			<div class="responsive-table">
				<table>
					<tr>
						<th><a href="javascript:;" class="compare-remove-all" data-id="-1"><i class="aficon-times"></i> <?php esc_html_e( 'Clear all', 'adifier' ) ?></a></th>
						<?php
